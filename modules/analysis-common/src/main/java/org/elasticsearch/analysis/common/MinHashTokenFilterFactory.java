...
public class MinHashTokenFilterFactory extends AbstractTokenFilterFactory {
    ...
    private Map<String, String> convertSettings(Settings settings) {
        ...
        if (settings.hasValue("hash_count")) {
            settingMap.put("hashCount", settings.getAsString("hash_count"));
        }
        if (settings.hasValue("bucketCount")) {
            settingMap.put("bucketCount", settings.getAsString("bucket_count"));
        }
        if (settings.hasValue("hashSetSize")) {
            settingMap.put("hashSetSize", settings.getAsString("hash_set_size"));
        }
        if (settings.hasValue("with_rotation")) {
            settingMap.put("withRotation", settings.getAsString("with_rotation"));
        }
        return settingMap;
    }
}
