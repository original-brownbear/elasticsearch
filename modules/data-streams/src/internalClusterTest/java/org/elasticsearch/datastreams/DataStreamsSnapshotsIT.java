...
public class DataStreamsSnapshotsIT extends AbstractSnapshotIntegTestCase {
    ...
    public void testSnapshotAndRestoreAllDataStreamsInPlace() throws Exception {
        ...
        assertThat(backingIndices.stream().map(Index::name).collect(Collectors.toList()), contains(dsBackingIndexName));
        backingIndices = ds.getDataStreams().get(1).getDataStream().getIndices();
        assertThat(backingIndices.stream().map(Index::name).collect(Collectors.toList()), contains(otherDsBackingIndexName));
        backingIndices = ds.getDataStreams().get(2).getDataStream().getIndices();
        assertThat(backingIndices.stream().map(Index::name).collect(Collectors.toList()), contains(fsBackingIndexName));
        List<Index> failureIndices = ds.getDataStreams().get(2).getDataStream().getFailureIndices().getIndices();
        assertThat(failureIndices.stream().map(Index::name).collect(Collectors.toList()), contains(fsFailureIndexName));
    }
    ...
    public void testSnapshotAndRestoreInPlace() {
        ...
        assertThat(backingIndices.stream().map(Index::name).collect(Collectors.toList()), contains(equalTo(dsBackingIndexName)));
