...
public class CustomRoleMappingRealm extends Realm implements CachingRealm {
    ...
    public void lookupUser(String username, ActionListener<User> listener) {
        ...
        if (USERNAME.equals(username)) {
            buildUser(username, ActionListener.wrap(u -> listener.onResponse(cache.computeIfAbsent(username, k -> u)), listener));
        } else {
    ...
    private void buildUser(String username, ActionListener<User> listener) {
        ...
        roleMapper.resolveRoles(
            data,
            ActionListener.wrap(roles -> listener.onResponse(new User(username, roles.toArray(String[]::new))), listener)
        );
    }
