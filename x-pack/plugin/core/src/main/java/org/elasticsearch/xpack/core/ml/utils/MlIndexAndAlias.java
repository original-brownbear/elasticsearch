...
public final class MlIndexAndAlias {
    ...
    public static void createIndexAndAliasIfNecessary(Client client,
        ...
        ActionListener<Boolean> indexCreatedListener = loggingListener.wrap((created, l) -> {
                    if (created) {
        ...
        if (concreteIndexNames.length == 0) {
            ...
            if (indexPointedByCurrentWriteAlias.get().getIndex().getName().equals(legacyIndexWithoutSuffix)) {
                createFirstConcreteIndex(
                    ...
                    alias,
                    false,
                    loggingListener.wrap(unused ->
                            updateWriteAlias(client, alias, legacyIndexWithoutSuffix, firstConcreteIndex, indexCreatedListener))
                );
                return;
            }
            logger.error(
    ...
    private static void waitForShardsReady(Client client, String index, ActionListener<Boolean> listener) {
        ...
        executeAsyncWithOrigin(
            ...
            ML_ORIGIN,
            healthRequest,
            listener.<ClusterHealthResponse>wrap((response, l) -> l.onResponse(response.isTimedOut() == false)),
    ...
    private static void updateWriteAlias(Client client,
        ...
        executeAsyncWithOrigin(client.threadPool().getThreadContext(),
            ML_ORIGIN,
            request,
            listener.<AcknowledgedResponse>wrap((resp, l) -> l.onResponse(resp.isAcknowledged())),
    ...
    public static void installIndexTemplateIfRequired(
        ...
        ActionListener<AcknowledgedResponse> innerListener = listener.wrap((response, l) -> {
