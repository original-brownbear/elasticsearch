...
public abstract class AsyncTwoPhaseIndexer<JobPosition, JobStats extends IndexerJobStats> {
    ...
    static class ScheduledRunnable {
        ...
        ScheduledRunnable(ThreadPool threadPool, TimeValue delay, Runnable command) {
            ...
            this.scheduled = threadPool.schedule(command, delay, ThreadPool.Names.GENERIC);
        }
        ...
        public void reschedule(TimeValue delay) {
            ...
            if (scheduled.cancel()) {
                if (delay.duration() > 0) {
                    scheduled = threadPool.schedule(command, delay, ThreadPool.Names.GENERIC);
                } else {
                    threadPool.executor(ThreadPool.Names.GENERIC).execute(command);
                }
            }
        }
