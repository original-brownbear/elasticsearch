...
public class RollupRequestTranslationTests extends ESTestCase {
    ...
    public void testBasicDateHisto() {
        ...
        assertThat(((SumAggregationBuilder) subAggs.get("the_avg._count")).field(), equalTo("avg_field.avg._count"));

        assertThat(subAggs.get("test_histo._count"), Matchers.instanceOf(SumAggregationBuilder.class));
        assertThat(((SumAggregationBuilder) subAggs.get("test_histo._count")).field(), equalTo("foo.date_histogram._count"));
    }
    ...
    public void testSimpleMetric() {
        ...
        if (i == 0) {
            translated = translateAggregation(new MaxAggregationBuilder("test_metric").field("foo"), namedWriteableRegistry);
            clazz = MaxAggregationBuilder.class;
            fieldName = "foo.max.value";
        } else if (i == 1) {
            translated = translateAggregation(new MinAggregationBuilder("test_metric").field("foo"), namedWriteableRegistry);
            clazz = MinAggregationBuilder.class;
            fieldName = "foo.min.value";
        } else if (i == 2) {
            translated = translateAggregation(new SumAggregationBuilder("test_metric").field("foo"), namedWriteableRegistry);
    ...
    public void testUnsupportedMetric() {
        IllegalArgumentException e = expectThrows(
            ...
            () -> translateAggregation(new StatsAggregationBuilder("test_metric").field("foo"), namedWriteableRegistry)
        );
        assertThat(
            e.getMessage(),
            equalTo(
                "Unable to translate aggregation tree into Rollup.  Aggregation [test_metric] is of type "
    ...
    public void testAvgMetric() {
        List<AggregationBuilder> translated = translateAggregation(
            new AvgAggregationBuilder("test_metric").field("foo"),
        ...
        assertThat(((SumAggregationBuilder) metrics.get("test_metric.value")).field(), equalTo("foo.avg.value"));

        assertThat(metrics.get("test_metric._count"), Matchers.instanceOf(SumAggregationBuilder.class));
        assertThat(((SumAggregationBuilder) metrics.get("test_metric._count")).field(), equalTo("foo.avg._count"));
    }
    ...
    public void testStringTerms() throws IOException {
        ...
        assertThat(((SumAggregationBuilder) subAggs.get("test_string_terms._count")).field(), equalTo("foo.terms._count"));
    }
    ...
    public void testBasicHisto() {
        ...
        assertThat(subAggs.get("test_histo._count"), Matchers.instanceOf(SumAggregationBuilder.class));
        assertThat(((SumAggregationBuilder) subAggs.get("test_histo._count")).field(), equalTo("foo.histogram._count"));
    }
    ...
    public void testUnsupportedAgg() {
        ...
        Exception e = expectThrows(RuntimeException.class, () -> translateAggregation(geo, namedWriteableRegistry));
        assertThat(
            e.getMessage(),
            equalTo(
                "Unable to translate aggregation tree into Rollup.  Aggregation [test_geo] is of type "
