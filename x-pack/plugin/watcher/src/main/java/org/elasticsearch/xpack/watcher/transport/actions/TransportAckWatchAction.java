                    ...
                    executeAsyncWithOrigin(
                        ...
                        delegate.<GetResponse>delegateFailureAndWrap((subDelegate, getResponse) -> {
                            if (getResponse.isExists() == false) {
                                ...
                                if (isChanged == false) {
                                    subDelegate.onResponse(new AckWatchResponse(watch.status()));
                                    return;
                                }
                                ...
                                executeAsyncWithOrigin(
                                    ...
                                    WATCHER_ORIGIN,
                                    updateRequest,
                                    subDelegate.<UpdateResponse>delegateFailureAndWrap(
                                        (l, updateResponse) -> l.onResponse(new AckWatchResponse(watch.status()))
                                    ),
                                    client::update
                                );
                            }
