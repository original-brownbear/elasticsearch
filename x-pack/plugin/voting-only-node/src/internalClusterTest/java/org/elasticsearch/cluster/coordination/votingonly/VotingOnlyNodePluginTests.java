...
public class VotingOnlyNodePluginTests extends ESIntegTestCase {
    ...
    public void testVotingOnlyNodeStats() throws Exception {
        ...
        assertBusy(() -> assertThat(clusterState().getLastCommittedConfiguration().getNodeIds(), hasSize(3)));
        assertThat(
    ...
    public void testPreferFullMasterOverVotingOnlyNodes() throws Exception {
        ...
        assertBusy(() -> assertThat(clusterState().getLastCommittedConfiguration().getNodeIds().size(), equalTo(3)));
        ...
        assertThat(VotingOnlyNodePlugin.isVotingOnlyNode(clusterState().nodes().getMasterNode()), equalTo(false));
    }
    ...
    public void testBootstrapOnlySingleVotingOnlyNode() throws Exception {
        ...
        internalCluster().startNode();
        assertBusy(() -> assertThat(clusterState().getNodes().getSize(), equalTo(2)));
        assertThat(VotingOnlyNodePlugin.isVotingOnlyNode(clusterState().nodes().getMasterNode()), equalTo(false));
    }
    ...
    public void testVotingOnlyNodesCannotBeMasterWithoutFullMasterNodes() throws Exception {
        ...
        internalCluster().startDataOnlyNodes(randomInt(2));
        assertBusy(() -> assertThat(clusterState().getLastCommittedConfiguration().getNodeIds().size(), equalTo(3)));
        final String oldMasterId = clusterState().nodes().getMasterNodeId();
        ...
        final String newMasterId = clusterState().nodes().getMasterNodeId();
