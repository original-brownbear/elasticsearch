...
public class TransportDeleteSamlServiceProviderAction
    ...
    protected void doExecute(Task task, final DeleteSamlServiceProviderRequest request,
        ...
        index.findByEntityId(entityId, listener.wrap((matchingDocuments, l) -> {
            if (matchingDocuments.isEmpty()) {
                ...
                index.deleteDocument(docInfo.version, request.getRefreshPolicy(), l.wrap((deleteResponse, ll) ->
                        ll.onResponse(new DeleteSamlServiceProviderResponse(deleteResponse, entityId))));
            } else {
