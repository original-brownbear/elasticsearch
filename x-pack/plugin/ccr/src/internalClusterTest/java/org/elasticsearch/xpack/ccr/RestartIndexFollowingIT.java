...
public class RestartIndexFollowingIT extends CcrIntegTestCase {
    ...
    private boolean isRemoteConnected() throws Exception {
        var infos = followerClient().execute(TransportRemoteInfoAction.ACTION, new RemoteInfoRequest()).get().getInfos();
    ...
    private void cleanRemoteCluster() throws Exception {
        ...
        assertBusy(() -> {
            List<RemoteConnectionInfo> infos = followerClient().execute(TransportRemoteInfoAction.ACTION, new RemoteInfoRequest())
                .get()
                .getInfos();
