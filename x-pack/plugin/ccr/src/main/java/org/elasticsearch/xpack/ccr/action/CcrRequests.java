...
public final class CcrRequests {
    ...
    public static void getIndexMetadata(Client client, Index index, long mappingVersion, long metadataVersion,
        ...
        client.admin().cluster().state(request, listener.wrap((response, wrappedListener) -> {
                if (response.getState() == null) { // timeout on wait_for_metadata_version
                    ...
                    if (timeoutSupplier.get().nanos() < 0) {
                        wrappedListener.onFailure(new IllegalStateException("timeout to get cluster state with" +
                            " metadata version [" + metadataVersion + "], mapping version [" + mappingVersion + "]"));
                    } else {
                        getIndexMetadata(client, index, mappingVersion, metadataVersion, timeoutSupplier, wrappedListener);
                    }
                } else {
                    ...
                    if (indexMetadata.getMappingVersion() >= mappingVersion) {
                        wrappedListener.onResponse(indexMetadata);
                        return;
                    }
                    if (timeoutSupplier.get().nanos() < 0) {
                        wrappedListener.onFailure(new IllegalStateException(
                            "timeout to get cluster state with mapping version [" + mappingVersion + "]"));
                    } else {
                        // ask for the next version.
                        getIndexMetadata(client, index, mappingVersion, metadata.version() + 1, timeoutSupplier, wrappedListener);
                    }
                }
            }));
    }
