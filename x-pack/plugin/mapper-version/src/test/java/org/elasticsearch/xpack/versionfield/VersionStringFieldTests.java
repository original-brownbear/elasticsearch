...
public class VersionStringFieldTests extends ESSingleNodeTestCase {
    ...
    public void testExactQueries() throws Exception {
        ...
        // match
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", ("1.0.0"))));
        assertHitCount(0, client().prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", "1.4.0")));
        assertHitCount(0, client().prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", "1.3.0")));
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", "1.3.0+build.1234567")));

        // term
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.termQuery("version", "1.0.0")));
        assertHitCount(0, client().prepareSearch(indexName).setQuery(QueryBuilders.termQuery("version", "1.4.0")));
        assertHitCount(0, client().prepareSearch(indexName).setQuery(QueryBuilders.termQuery("version", "1.3.0")));
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.termQuery("version", "1.3.0+build.1234567")));

        // terms
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.termsQuery("version", "1.0.0", "1.3.0")));
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.termsQuery("version", "1.4.0", "1.3.0+build.1234567")));

        // phrase query (just for keyword compatibility)
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.matchPhraseQuery("version", "2.1.0-alpha.beta")));
    }

    public void testRangeQueries() throws Exception {
        String indexName = setUpIndex("test");
        assertHitCount(4, client().prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("1.0.0").to("3.0.0")));
        assertHitCount(3, client().prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("1.1.0").to("3.0.0")));
        assertHitCount(
            3,
            client().prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("0.1.0").to("2.1.0-alpha.beta"))
        );
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("2.1.0").to("3.0.0")));
        assertHitCount(0, client().prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("3.0.0").to("4.0.0")));
        assertHitCount(
            2,
        ...
        assertHitCount(3, client().prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("1.0.0", false).to("3.0.0")));
        assertHitCount(3, client().prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("1.0.0").to("2.1.0", false)));

        // open ranges
        assertHitCount(4, client().prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("1.4.0")));
        assertHitCount(2, client().prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").to("1.4.0")));
    }

    public void testPrefixQuery() throws IOException {
        String indexName = setUpIndex("test");
        // prefix
        assertHitCount(3, client().prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "1")));
        assertHitCount(2, client().prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "2.1")));
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "2.1.0-")));
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "1.3.0+b")));
        assertHitCount(3, client().prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "2")));
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "21")));
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "21.")));
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "21.1")));
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "21.11")));

        // test case sensitivity / insensitivity
        assertHitCount(0, client().prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "2.1.0-A")));
    ...
    public void testRegexQuery() throws Exception {
        ...
        }, client().prepareSearch(indexName).setQuery(QueryBuilders.regexpQuery("version", "2.*0")));
        ...
        }, client().prepareSearch(indexName).setQuery(QueryBuilders.regexpQuery("version", "1.[0-9].[0-9].*")));
        ...
        assertResponse(response -> {
            ...
            assertEquals("1.0.0alpha2.1.0-rc.1", response.getHits().getHits()[0].getSourceAsMap().get("version"));
            assertEquals("2.1.0-alpha.beta", response.getHits().getHits()[1].getSourceAsMap().get("version"));
        }, client().prepareSearch(indexName).setQuery(QueryBuilders.regexpQuery("version", ".*alpha.*")));

        assertHitCount(0, client().prepareSearch(indexName).setQuery(QueryBuilders.regexpQuery("version", ".*Alpha.*")));
    ...
    public void testFuzzyQuery() throws Exception {
        ...
        }, client().prepareSearch(indexName).setQuery(QueryBuilders.fuzzyQuery("version", "2.3.0")));
    }
    ...
    public void testWildcardQuery() throws Exception {
        ...
        // test case sensitivity / insensitivity
        assertHitCount(0, client().prepareSearch(indexName).setQuery(QueryBuilders.wildcardQuery("version", "*Alpha*")));
    ...
    private void checkWildcardQuery(String indexName, String query, String... expectedResults) {
        ...
        }, client().prepareSearch(indexName).setQuery(QueryBuilders.wildcardQuery("version", query)));
    }
    ...
    public void testStoreMalformed() throws Exception {
        ...
        }, client().prepareSearch(indexName).addDocValueField("version"));

        // exact match for malformed term
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", "1.invalid.0")));
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", "2.2.0-badchar!")));
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", "")));
        ...
        assertHitCount(1, client().prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").to("3.0.0")));
        assertHitCount(3, client().prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("3.0.0")));

        // using the empty string as lower bound should return all "invalid" versions
        assertHitCount(3, client().prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("")));
    }
    ...
    public void testMultiValues() throws Exception {
        ...
        assertResponse(response -> {
            ...
            assertEquals("2", response.getHits().getAt(1).getId());
            assertEquals("3", response.getHits().getAt(2).getId());
        }, client().prepareSearch(indexName).addSort("version", SortOrder.ASC));

        assertResponse(response -> {
            assertEquals(1, response.getHits().getTotalHits().value);
            assertEquals("1", response.getHits().getAt(0).getId());
        }, client().prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", "3.0.0")));

        assertResponse(response -> {
            assertEquals(1, response.getHits().getTotalHits().value);
            assertEquals("2", response.getHits().getAt(0).getId());
        }, client().prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", "4.alpha.0")));
