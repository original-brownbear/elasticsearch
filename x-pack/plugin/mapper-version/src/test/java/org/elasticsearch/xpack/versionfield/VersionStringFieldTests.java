...
public class VersionStringFieldTests extends ESSingleNodeTestCase {
    ...
    public void testExactQueries() throws Exception {
        ...
        // match
        SearchResponse response = prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", ("1.0.0"))).get();
        assertEquals(1, response.getHits().getTotalHits().value);
        response = prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", "1.4.0")).get();
        assertEquals(0, response.getHits().getTotalHits().value);
        response = prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", "1.3.0")).get();
        assertEquals(0, response.getHits().getTotalHits().value);
        response = prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", "1.3.0+build.1234567")).get();
        assertEquals(1, response.getHits().getTotalHits().value);

        // term
        response = prepareSearch(indexName).setQuery(QueryBuilders.termQuery("version", "1.0.0")).get();
        assertEquals(1, response.getHits().getTotalHits().value);
        response = prepareSearch(indexName).setQuery(QueryBuilders.termQuery("version", "1.4.0")).get();
        assertEquals(0, response.getHits().getTotalHits().value);
        response = prepareSearch(indexName).setQuery(QueryBuilders.termQuery("version", "1.3.0")).get();
        assertEquals(0, response.getHits().getTotalHits().value);
        response = prepareSearch(indexName).setQuery(QueryBuilders.termQuery("version", "1.3.0+build.1234567")).get();
        assertEquals(1, response.getHits().getTotalHits().value);

        // terms
        response = prepareSearch(indexName).setQuery(QueryBuilders.termsQuery("version", "1.0.0", "1.3.0")).get();
        assertEquals(1, response.getHits().getTotalHits().value);
        response = prepareSearch(indexName).setQuery(QueryBuilders.termsQuery("version", "1.4.0", "1.3.0+build.1234567")).get();
        assertEquals(1, response.getHits().getTotalHits().value);

        // phrase query (just for keyword compatibility)
        response = prepareSearch(indexName).setQuery(QueryBuilders.matchPhraseQuery("version", "2.1.0-alpha.beta")).get();
        assertEquals(1, response.getHits().getTotalHits().value);
    }

    public void testRangeQueries() throws Exception {
        String indexName = setUpIndex("test");
        SearchResponse response = prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("1.0.0").to("3.0.0")).get();
        assertEquals(4, response.getHits().getTotalHits().value);
        response = prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("1.1.0").to("3.0.0")).get();
        assertEquals(3, response.getHits().getTotalHits().value);
        response = prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("0.1.0").to("2.1.0-alpha.beta")).get();
        assertEquals(3, response.getHits().getTotalHits().value);
        response = prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("2.1.0").to("3.0.0")).get();
        assertEquals(1, response.getHits().getTotalHits().value);
        response = prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("3.0.0").to("4.0.0")).get();
        assertEquals(0, response.getHits().getTotalHits().value);
        response = prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("1.3.0+build.1234569").to("3.0.0")).get();
        assertEquals(2, response.getHits().getTotalHits().value);

        // ranges excluding edges
        response = prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("1.0.0", false).to("3.0.0")).get();
        assertEquals(3, response.getHits().getTotalHits().value);
        response = prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("1.0.0").to("2.1.0", false)).get();
        assertEquals(3, response.getHits().getTotalHits().value);

        // open ranges
        response = prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("1.4.0")).get();
        assertEquals(4, response.getHits().getTotalHits().value);

        response = prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").to("1.4.0")).get();
        assertEquals(2, response.getHits().getTotalHits().value);
    }
    ...
    public void testPrefixQuery() throws IOException {
        String indexName = setUpIndex("test");
        // prefix
        SearchResponse response = prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "1")).get();
        assertEquals(3, response.getHits().getTotalHits().value);

        response = prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "2.1")).get();
        assertEquals(2, response.getHits().getTotalHits().value);

        response = prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "2.1.0-")).get();
        assertEquals(1, response.getHits().getTotalHits().value);

        response = prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "1.3.0+b")).get();
        assertEquals(1, response.getHits().getTotalHits().value);

        response = prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "2")).get();
        assertEquals(3, response.getHits().getTotalHits().value);

        response = prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "21")).get();
        assertEquals(1, response.getHits().getTotalHits().value);

        response = prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "21.")).get();
        assertEquals(1, response.getHits().getTotalHits().value);

        response = prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "21.1")).get();
        assertEquals(1, response.getHits().getTotalHits().value);

        response = prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "21.11")).get();
        assertEquals(1, response.getHits().getTotalHits().value);

        // test case sensitivity / insensitivity
        response = prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "2.1.0-A")).get();
        assertEquals(0, response.getHits().getTotalHits().value);

        response = prepareSearch(indexName).setQuery(QueryBuilders.prefixQuery("version", "2.1.0-A").caseInsensitive(true)).get();
        assertEquals(1, response.getHits().getTotalHits().value);
    ...
    public void testSort() throws IOException {
        ...
        SearchResponse response = prepareSearch(indexName).setQuery(QueryBuilders.matchAllQuery()).addSort("version", SortOrder.DESC).get();
        ...
        response = prepareSearch(indexName).setQuery(QueryBuilders.matchAllQuery()).addSort("version", SortOrder.ASC).get();
        assertEquals(8, response.getHits().getTotalHits().value);
    ...
    public void testRegexQuery() throws Exception {
        ...
        SearchResponse response = prepareSearch(indexName).setQuery(QueryBuilders.regexpQuery("version", "2.*0")).get();
        assertEquals(2, response.getHits().getTotalHits().value);
        ...
        response = prepareSearch(indexName).setQuery(QueryBuilders.regexpQuery("version", "<0-10>.<0-10>.*al.*")).get();
        assertEquals(2, response.getHits().getTotalHits().value);
        ...
        response = prepareSearch(indexName).setQuery(QueryBuilders.regexpQuery("version", "1.[0-9].[0-9].*")).get();
        assertEquals(2, response.getHits().getTotalHits().value);
        assertEquals("1.0.0alpha2.1.0-rc.1", response.getHits().getHits()[0].getSourceAsMap().get("version"));
        assertEquals("1.3.0+build.1234567", response.getHits().getHits()[1].getSourceAsMap().get("version"));

        // test case sensitivity / insensitivity
        response = prepareSearch(indexName).setQuery(QueryBuilders.regexpQuery("version", ".*alpha.*")).get();
        assertEquals(2, response.getHits().getTotalHits().value);
        assertEquals("1.0.0alpha2.1.0-rc.1", response.getHits().getHits()[0].getSourceAsMap().get("version"));
        assertEquals("2.1.0-alpha.beta", response.getHits().getHits()[1].getSourceAsMap().get("version"));
        ...
        response = prepareSearch(indexName).setQuery(QueryBuilders.regexpQuery("version", ".*Alpha.*").caseInsensitive(true)).get();
        assertEquals(2, response.getHits().getTotalHits().value);
        assertEquals("1.0.0alpha2.1.0-rc.1", response.getHits().getHits()[0].getSourceAsMap().get("version"));
        assertEquals("2.1.0-alpha.beta", response.getHits().getHits()[1].getSourceAsMap().get("version"));
    }
    ...
    public void testFuzzyQuery() throws Exception {
        ...
        SearchResponse response = prepareSearch(indexName).setQuery(QueryBuilders.fuzzyQuery("version", "2.3.0")).get();
        assertEquals(3, response.getHits().getTotalHits().value);
        assertEquals("2.1.0", response.getHits().getHits()[0].getSourceAsMap().get("version"));
        assertEquals("2.33.0", response.getHits().getHits()[1].getSourceAsMap().get("version"));
    ...
    public void testWildcardQuery() throws Exception {
        ...
        // test case sensitivity / insensitivity
        SearchResponse response = prepareSearch(indexName).setQuery(QueryBuilders.wildcardQuery("version", "*Alpha*")).get();
        assertEquals(0, response.getHits().getTotalHits().value);

        response = prepareSearch(indexName).setQuery(QueryBuilders.wildcardQuery("version", "*Alpha*").caseInsensitive(true)).get();
        assertEquals(2, response.getHits().getTotalHits().value);
    ...
    private void checkWildcardQuery(String indexName, String query, String... expectedResults) {
        SearchResponse response = prepareSearch(indexName).setQuery(QueryBuilders.wildcardQuery("version", query)).get();
    ...
    public void testStoreMalformed() throws Exception {
        ...
        SearchResponse response = prepareSearch(indexName).addDocValueField("version").get();
        assertEquals(4, response.getHits().getTotalHits().value);
        ...
        response = prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", "1.invalid.0")).get();
        assertEquals(1, response.getHits().getTotalHits().value);

        response = prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", "2.2.0-badchar!")).get();
        assertEquals(1, response.getHits().getTotalHits().value);

        response = prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", "")).get();
        assertEquals(1, response.getHits().getTotalHits().value);

        // also should appear in terms aggs
        response = prepareSearch(indexName).addAggregation(AggregationBuilders.terms("myterms").field("version")).get();
        ...
        response = prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").to("3.0.0")).get();
        assertEquals(1, response.getHits().getTotalHits().value);
        response = prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("3.0.0")).get();
        assertEquals(3, response.getHits().getTotalHits().value);

        // using the empty string as lower bound should return all "invalid" versions
        response = prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("")).get();
        assertEquals(3, response.getHits().getTotalHits().value);
    }
    ...
    public void testAggs() throws Exception {
        ...
        // cardinality
        response = prepareSearch(indexName).addAggregation(AggregationBuilders.cardinality("myterms").field("version")).get();
        Cardinality card = response.getAggregations().get("myterms");
        assertEquals(5, card.getValue());

        // string stats
        response = prepareSearch(indexName).addAggregation(AnalyticsAggregationBuilders.stringStats("stats").field("version")).get();
    ...
    public void testMultiValues() throws Exception {
        ...
        SearchResponse response = prepareSearch(indexName).addSort("version", SortOrder.ASC).get();
        assertEquals(3, response.getHits().getTotalHits().value);
        assertEquals("1", response.getHits().getAt(0).getId());
        assertEquals("2", response.getHits().getAt(1).getId());
        assertEquals("3", response.getHits().getAt(2).getId());

        response = prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", "3.0.0")).get();
        assertEquals(1, response.getHits().getTotalHits().value);
        assertEquals("1", response.getHits().getAt(0).getId());

        response = prepareSearch(indexName).setQuery(QueryBuilders.matchQuery("version", "4.alpha.0")).get();
        assertEquals(1, response.getHits().getTotalHits().value);
        assertEquals("2", response.getHits().getAt(0).getId());

        // range
        response = prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").to("1.5.0")).get();
        assertEquals(1, response.getHits().getTotalHits().value);

        response = prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("1.5.0")).get();
        assertEquals(3, response.getHits().getTotalHits().value);

        response = prepareSearch(indexName).setQuery(QueryBuilders.rangeQuery("version").from("5.0.0").to("6.0.0")).get();
        assertEquals(1, response.getHits().getTotalHits().value);
    }
}
