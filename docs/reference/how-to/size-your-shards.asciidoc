...
==== Limit the number of shards per node and indices in a cluster

The number of shards a data node can hold depends on the data in those shards as well as the search and indexing load
on them.
The more fields the mapping of an index has, the more heap memory its shards will consume regardless of the amount of
data in the shard.
