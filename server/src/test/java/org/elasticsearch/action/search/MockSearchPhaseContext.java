...
public final class MockSearchPhaseContext extends AbstractSearchAsyncAction<SearchPhaseResult> {
    ...
    @Override
    public void onPhaseFailure(String currentPhaseName, String msg, Throwable cause) {
    ...
    @Override
    public void executeNextPhase(String currentPhaseName, Supplier<SearchPhase> nextPhaseSupplier) {
        var nextPhase = nextPhaseSupplier.get();
        try {
            nextPhase.run();
        } catch (Exception e) {
            onPhaseFailure(currentPhaseName, "phase failed", e);
        }
    }
