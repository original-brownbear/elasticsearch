...
public class DateMathExpressionResolverTests extends ESTestCase {
    ...
    public void testNormal() throws Exception {
        ...
        List<String> result = IndexNameExpressionResolver.resolveDateMathExpressions(context, indexExpressions);
    ...
    public void testEmpty() throws Exception {
        List<String> result = IndexNameExpressionResolver.resolveDateMathExpressions(context, Collections.<String>emptyList());
        assertThat(result.size(), equalTo(0));
    }

    public void testExpression_Static() throws Exception {
        List<String> result = IndexNameExpressionResolver.resolveDateMathExpressions(context, Arrays.asList("<.marvel-test>"));
        assertThat(result.size(), equalTo(1));
        assertThat(result.get(0), equalTo(".marvel-test"));
    }

    public void testExpression_MultiParts() throws Exception {
        List<String> result = IndexNameExpressionResolver.resolveDateMathExpressions(
            context,
            Arrays.asList("<.text1-{now/d}-text2-{now/M}>")
        );
        assertThat(result.size(), equalTo(1));
        assertThat(
            result.get(0),
    ...
    public void testExpression_CustomFormat() throws Exception {
        List<String> results = IndexNameExpressionResolver.resolveDateMathExpressions(
            context,
            Arrays.asList("<.marvel-{now/d{yyyy.MM.dd}}>")
        );
    ...
    public void testExpression_EscapeStatic() throws Exception {
        List<String> result = IndexNameExpressionResolver.resolveDateMathExpressions(context, Arrays.asList("<.mar\\{v\\}el-{now/d}>"));
        assertThat(result.size(), equalTo(1));
    ...
    public void testExpression_EscapeDateFormat() throws Exception {
        List<String> result = IndexNameExpressionResolver.resolveDateMathExpressions(
            context,
            Arrays.asList("<.marvel-{now/d{'\\{year\\}'yyyy}}>")
        );
        assertThat(result.size(), equalTo(1));
    ...
    public void testExpressionInvalidUnescaped() throws Exception {
        Exception e = expectThrows(
            ...
            () -> IndexNameExpressionResolver.resolveDateMathExpressions(context, Arrays.asList("<.mar}vel-{now/d}>"))
        );
    ...
    public void testExpressionInvalidDateMathFormat() throws Exception {
        Exception e = expectThrows(
            ElasticsearchParseException.class,
            () -> IndexNameExpressionResolver.resolveDateMathExpressions(context, Arrays.asList("<.marvel-{now/d{}>"))
        );
        assertThat(e.getMessage(), containsString("invalid dynamic name expression"));
