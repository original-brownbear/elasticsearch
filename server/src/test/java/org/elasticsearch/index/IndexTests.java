...
public class IndexTests extends AbstractXContentSerializingTestCase<Index> {
    ...
    public static Index mutate(Index instance) {
        return switch (randomInt(1)) {
            case 0 -> {
                yield new Index(randomValueOtherThan(instance.name(), ESTestCase::randomIdentifier), instance.uuid());
            }
            case 1 -> {
                yield new Index(instance.name(), randomValueOtherThan(instance.uuid(), UUIDs::randomBase64UUID));
            }
