...
abstract class OutboundMessage extends NetworkMessage {
    ...
    BytesReference serialize(BytesStream bytesStream) throws IOException {
        ...
        final long lastPosition = bytesStream.position();
        final int contentSize = Math.toIntExact(lastPosition - TcpHeader.headerSize(version));
        bytesStream.seek(0);
        TcpHeader.writeHeader(bytesStream, requestId, status, version, contentSize, variableHeaderLength);
        bytesStream.seek(lastPosition);
        return reference;
    }
