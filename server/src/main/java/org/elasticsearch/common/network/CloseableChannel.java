...
public interface CloseableChannel extends Releasable {
    ...
    /**
     * Closes the channels and block until closing completes.
     *
    ...
    static <C extends CloseableChannel> void closeChannels(List<C> channels) {
        if (channels.isEmpty() == false) {
            ...
            final var f = PlainActionFuture.<Collection<Void>>newFuture();
            final var closeListener = new GroupedActionListener<>(f, channels.size());
