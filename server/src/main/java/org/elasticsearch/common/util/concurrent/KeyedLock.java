...
public final class KeyedLock<T> {
    ...
    public Releasable acquire(T key) {
        Releasable newLock = null;
        while (newLock == null) {
            KeyLock perNodeLock = map.get(key);
            if (perNodeLock == null) {
                newLock = tryCreateNewLock(key);
            } else {
                newLock = tryLockExisting(key, perNodeLock);
            }
        }
        return newLock;
    }

    private Releasable tryLockExisting(T key, KeyLock perNodeLock) {
        if (perNodeLock.count.updateAndGet(i -> i == 0 ? 0 : i + 1) > 0) {
            perNodeLock.lock();
