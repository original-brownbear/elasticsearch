    ...
    public void restoreSnapshot(final RestoreSnapshotRequest request, final ActionListener<RestoreCompletionResponse> listener) {
        try {
            ...
            repository.getSnapshotInfo(snapshotId, ActionListener.wrap(snapshotInfo -> {
                    ...
                    final List<String> indicesInSnapshot =
                                ...
                                for (Map.Entry<String, String> indexEntry : indices.entrySet()) {
                                    ...
                                    try {
                                        ...
                                        throw new SnapshotRestoreException(
                                            snapshot, "cannot restore index [" + index + "] because it cannot be " + "upgraded", ex);
                                    }
                                    ...
                                    if (currentIndexMetaData == null) {
                                        ...
                                        indexMdBuilder.version(
                                            Math.max(snapshotIndexMetaData.getVersion(), currentIndexMetaData.getVersion() + 1));
                                        ...
                                        for (int shard = 0; shard < snapshotIndexMetaData.getNumberOfShards(); shard++) {
                                            indexMdBuilder.primaryTerm(shard,
                                                Math.max(
                                                    snapshotIndexMetaData.primaryTerm(shard), currentIndexMetaData.primaryTerm(shard)));
                                        }
                        ...
                        private void validateExistingIndex(IndexMetaData currentIndexMetaData, IndexMetaData snapshotIndexMetaData,
                            ...
                            if (currentIndexMetaData.getState() != IndexMetaData.State.CLOSE) {
                                ...
                                throw new SnapshotRestoreException(snapshot, "cannot restore index [" + renamedIndex +
                                    ...
                                    "with same name already exists in the cluster. Either close or delete the existing index " +
                                    "or restore the index under a different name by providing a rename pattern and replacement name");
                            }
