...
public final class HealthNodeTaskExecutor extends PersistentTasksExecutor<HealthNodeTaskParams> {
    ...
    void shuttingDown(ClusterChangedEvent event) {
        ...
        if (event.previousState().metadata().nodeShutdowns().contains(localNodeId) == false
            && event.state().metadata().nodeShutdowns().contains(localNodeId)) {
            abortTaskIfApplicable("node [{" + localNode.getName() + "}{" + localNodeId + "}] shutting down");
        }
    }
