...
public final class DataStream implements SimpleDiffable<DataStream>, ToXContentObject, IndexAbstraction {
        ...
        if (indexMode == IndexMode.TIME_SERIES) {
            ...
            var startAndEndTimes = backingIndices.indices.stream().map(index -> {
                ...
                .map(im -> {
                    ...
                    return new Tuple<>(index.name(), new Tuple<>(start, end));
                })
    ...
    public DataStream snapshot(Collection<String> indicesInSnapshot) {
        ...
        if (reconciledIndices.removeIf(x -> { return indicesInSnapshot.contains(x.name()) == false; }) == false) {
            return this;
        }
    ...
    public boolean isIndexManagedByDataStreamLifecycle(Index index, Function<String, IndexMetadata> indexMetadataSupplier) {
        if (backingIndices.containsIndex(index.name()) == false) {
            if (failureIndices.containsIndex(index.name()) == false) {
                return false;
            }
        }
        IndexMetadata indexMetadata = indexMetadataSupplier.apply(index.name());
    ...
    public static class DataStreamIndices {
        ...
        private Set<String> getLookup() {
            if (lookup == null) {
                lookup = indices.stream().map(index -> index.name()).collect(Collectors.toSet());
            }
            return lookup;
        }
