    ...
    *                       Elasticsearch version when the index was created.
    ...
    SystemIndexDescriptor(
        ...
        if (type.isManaged()) {
            ...
            Strings.requireNonEmpty(mappings == null ? null : mappings.string(), "Must supply mappings for a managed system index");
    ...
    public static class Builder {
        ...
        public Builder setMappings(XContentBuilder mappingsBuilder) {
            try {
                mappings = mappingsBuilder == null ? null : CompressedXContent.fromJSON(Strings.toString(mappingsBuilder));
            } catch (IOException e) {
                throw new UncheckedIOException(e);
            }
            return this;
    ...
    @SuppressWarnings("unchecked")
    private static Version extractVersionFromMappings(CompressedXContent mappings, String versionMetaKey) {
        final Map<String, Object> mappingsMap;
        try {
            mappingsMap = XContentHelper.convertToMap(XContentType.JSON.xContent(), mappings.uncompressed().streamInput(), false);
        } catch (IOException e) {
            throw new UncheckedIOException(e);
        }
