...
public class TransportSearchAction extends HandledTransportAction<SearchRequest, SearchResponse> {
    ...
    void executeRequest(
        ...
        if (shouldOpenPIT(source)) {
            ...
            doExecuteOpenPITRequest(task, original, listener, searchPhaseProvider, source);
        } else {
            doExecuteSearch(task, original, listener, searchPhaseProvider, source);
        }
    }

    private void doExecuteOpenPITRequest(
        SearchTask task,
        SearchRequest original,
        ActionListener<SearchResponse> listener,
        Function<ActionListener<SearchResponse>, SearchPhaseProvider> searchPhaseProvider,
    ...
    private void doExecuteSearch(
        ...
        Rewriteable.rewriteAndFetch(
            ...
            searchService.getRewriteContext(
                ...
                resolvedIndices,
                original.pointInTimeBuilder(),
                source != null && source.explain() != null && source.explain()
            ),
            listener.delegateFailureAndWrap((delegate, rewritten) -> {
                ...
                if (resolvedIndices.getRemoteClusterIndices().isEmpty()) {
                    ...
                    doExecuteSearchWithRemoteIndices(
                        task,
                        searchPhaseProvider,
                        delegate,
                        rewritten,
    ...
    private void doExecuteSearchWithRemoteIndices(
        SearchTask task,
        Function<ActionListener<SearchResponse>, SearchPhaseProvider> searchPhaseProvider,
