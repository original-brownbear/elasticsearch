...
public class TransportGetSnapshotsAction extends TransportMasterNodeAction<GetSnapshotsRequest, GetSnapshotsResponse> {
    ...
    private void getMultipleReposSnapshotInfo(List<RepositoryMetaData> repos, String[] snapshots, boolean ignoreUnavailable,
        ...
        for (final RepositoryMetaData repo : repos) {
            threadPool.generic().execute(ActionRunnable.wrap(ActionListener.delegateResponse(groupedActionListener, (l, e) -> {
                if (e instanceof ElasticsearchException) {
                    l.onResponse(GetSnapshotsResponse.Response.error(repo.name(), (ElasticsearchException) e));
                } else {
                    l.onFailure(e);
                }
            }), l -> getSingleRepoSnapshotInfo(repo.name(), snapshots, ignoreUnavailable, verbose, ActionListener.wrap(
                snapshotInfos -> l.onResponse(GetSnapshotsResponse.Response.snapshots(repo.name(), snapshotInfos)), l::onFailure))));
        }
    }
