...
public final class AutoCreateAction extends ActionType<CreateIndexResponse> {
    ...
    public static final class TransportAction extends TransportMasterNodeAction<CreateIndexRequest, CreateIndexResponse> {
        ...
        protected void masterOperation(Task task,
            ...
            ActionListener<AcknowledgedResponse> listener = finalListener.wrap((wrapped, response) -> {
                    ...
                    if (response.isAcknowledged()) {
                        activeShardsObserver.waitForActiveShards(
                            ...
                            request.timeout(),
                            shardsAcked -> wrapped.onResponse(new CreateIndexResponse(true, shardsAcked, indexName)),
                            wrapped::onFailure
                        );
                    } else {
                        wrapped.onResponse(new CreateIndexResponse(false, false, indexName));
                    }
                });
