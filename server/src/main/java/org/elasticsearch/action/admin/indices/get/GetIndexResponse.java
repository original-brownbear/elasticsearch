...
public class GetIndexResponse extends ActionResponse implements ToXContentObject {
    ...
    GetIndexResponse(StreamInput in) throws IOException {
        ...
        mappings = in.readImmutableStringKeyedMap(in.getVersion().before(Version.V_8_0_0) ? i -> {
        ...
        aliases = in.readImmutableStringKeyedMap(i -> i.readList(AliasMetadata::new));
        settings = in.readImmutableStringKeyedMap(Settings::readSettingsFromStream);
        defaultSettings = in.readImmutableStringKeyedMap(Settings::readSettingsFromStream);
        dataStreams = in.readImmutableStringKeyedMap(StreamInput::readOptionalString);
    }
