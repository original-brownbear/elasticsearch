...
public class ClusterSearchShardsIT extends ESIntegTestCase {
    ...
    public void testSingleShardAllocation() throws Exception {
        client().admin()
            .indices()
            .prepareCreate("test")
            .setSettings(shardsAndReplicas(1, 0).put("index.routing.allocation.include.tag", "A"))
            .execute()
            .actionGet();
        ensureGreen();
    ...
    public void testMultipleShardsSingleNodeAllocation() throws Exception {
        client().admin()
            .indices()
            .prepareCreate("test")
            .setSettings(shardsAndReplicas(4, 0).put("index.routing.allocation.include.tag", "A"))
            .execute()
            .actionGet();
        ensureGreen();
    ...
    public void testMultipleIndicesAllocation() throws Exception {
        client().admin().indices().prepareCreate("test1").setSettings(shardsAndReplicas(4, 1)).execute().actionGet();
        client().admin().indices().prepareCreate("test2").setSettings(shardsAndReplicas(4, 1)).execute().actionGet();
        client().admin()
            .indices()
            .prepareAliases()
