...
public class UpdateShardAllocationSettingsIT extends ESIntegTestCase {
    ...
    public void testEnableRebalance() {
        ...
        assertAcked(prepareCreate("test").setSettings(shardsAndReplicas(numShards, 0)));
        assertAcked(prepareCreate("test_1").setSettings(shardsAndReplicas(numShards, 0)));
        ensureGreen();
    ...
    public void testUpdateSameHostSetting() {
        ...
        client().admin().indices().prepareCreate(indexName).setSettings(shardsAndReplicas(1, 1)).get();
