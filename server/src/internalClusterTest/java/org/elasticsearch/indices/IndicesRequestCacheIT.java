...
public class IndicesRequestCacheIT extends ESIntegTestCase {
    ...
    public void testQueryRewriteDatesWithNow() throws Exception {
        ...
        assertAcked(indicesAdmin().prepareCreate("index-1").setMapping("d", "type=date").setSettings(settings));
        assertAcked(indicesAdmin().prepareCreate("index-2").setMapping("d", "type=date").setSettings(settings));
        assertAcked(indicesAdmin().prepareCreate("index-3").setMapping("d", "type=date").setSettings(settings));
    ...
    public void testCanCache() throws Exception {
        ...
        Settings settings = indexSettings(2, 0).put(IndicesRequestCache.INDEX_CACHE_REQUEST_ENABLED_SETTING.getKey(), true)
            ...
            .build();
        assertAcked(indicesAdmin().prepareCreate("index").setMapping("s", "type=date").setSettings(settings));
        indexRandom(
            true,
            client.prepareIndex("index").setId("1").setRouting("1").setSource("s", "2016-03-19"),
            client.prepareIndex("index").setId("2").setRouting("1").setSource("s", "2016-03-20"),
