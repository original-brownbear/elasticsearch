...
public class ReplicaShardAllocatorSyncIdIT extends ESIntegTestCase {
    ...
    public void testPreferCopyCanPerformNoopRecovery() throws Exception {
        ...
        assertAcked(
            client().admin()
                ...
                .setSettings(
                    shardsAndReplicas(1, 1).put(IndexSettings.INDEX_SOFT_DELETES_RETENTION_LEASE_PERIOD_SETTING.getKey(), "1ms") // expire
                                                                                                                                 // PRRLs
                                                                                                                                 // quickly
    ...
    public void testSimulateRecoverySourceOnOldNode() throws Exception {
        ...
        String indexName = "test";
        assertAcked(client().admin().indices().prepareCreate(indexName).setSettings(shardsAndReplicas(1, 0)));
        ensureGreen(indexName);
        if (randomBoolean()) {
            indexRandom(
                randomBoolean(),
