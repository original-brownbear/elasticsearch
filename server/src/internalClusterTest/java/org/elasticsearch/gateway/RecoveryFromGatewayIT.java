...
public class RecoveryFromGatewayIT extends ESIntegTestCase {
    ...
    public void testSingleNodeNoFlush() throws Exception {
        ...
        assertAcked(prepareCreate("test").setSettings(shardsAndReplicas(numberOfShards(), randomIntBetween(0, 1))).setMapping(mapping));
    ...
    public void testMessyElectionsStillMakeClusterGoGreen() throws Exception {
        ...
        createIndex("test", shardsAndReplicas(1, 0).put(UnassignedInfo.INDEX_DELAYED_NODE_LEFT_TIMEOUT_SETTING.getKey(), "100ms").build());
        ensureGreen("test");
        internalCluster().fullRestart();
        ensureGreen("test");
    }
