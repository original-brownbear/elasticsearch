...
public class SearchScrollIT extends ESIntegTestCase {
    ...
    public void testSimpleScrollQueryThenFetch() throws Exception {
        admin().indices().prepareCreate("test").setSettings(Settings.builder().put("index.number_of_shards", 3)).get();
    ...
    public void testScrollAndUpdateIndex() throws Exception {
        admin().indices().prepareCreate("test").setSettings(Settings.builder().put("index.number_of_shards", 5)).get();
        clusterAdmin().prepareHealth().setWaitForEvents(Priority.LANGUID).setWaitForGreenStatus().get();
    ...
    public void testCloseAndReopenOrDeleteWithActiveScroll() {
        ...
        if (randomBoolean()) {
            ...
            assertAcked(admin().indices().prepareOpen("test"));
            ensureGreen("test");
        } else {
            assertAcked(admin().indices().prepareDelete("test"));
        }
    }
