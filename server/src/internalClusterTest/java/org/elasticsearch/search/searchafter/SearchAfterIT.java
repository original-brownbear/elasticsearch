...
public class SearchAfterIT extends ESIntegTestCase {
    ...
    public void testsShouldFail() throws Exception {
        assertAcked(admin().indices().prepareCreate("test").setMapping("field1", "type=long", "field2", "type=keyword").get());
        ensureGreen();
    ...
    public void testWithNullStrings() throws InterruptedException {
        assertAcked(admin().indices().prepareCreate("test").setMapping("field2", "type=keyword").get());
        ensureGreen();
        indexRandom(
            true,
    ...
    private void createIndexMappingsFromObjectType(String indexName, List<Object> types) {
        CreateIndexRequestBuilder indexRequestBuilder = admin().indices().prepareCreate(indexName);
    ...
    public void testScrollAndSearchAfterWithBigIndex() {
        ...
        assertAcked(admin().indices().prepareCreate("test").setSettings(indexSettings).setMapping("""
