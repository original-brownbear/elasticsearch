...
public class ForceMergeIT extends ESIntegTestCase {
    ...
    public void testForceMergeUUIDConsistent() throws IOException {
        ...
        final ForceMergeResponse forceMergeResponse = admin().indices().prepareForceMerge(index).setMaxNumSegments(1).get();
        ...
        final FlushResponse flushResponse = admin().indices().prepareFlush(index).setForce(true).get();
