...
public class CreateIndexIT extends ESIntegTestCase {
    ...
    public void testNonNestedMappings() throws Exception {
        ...
        GetMappingsResponse response = admin().indices().prepareGetMappings("test").get();
    ...
    public void testCreateIndexWithMetadataBlocks() {
        ...
        assertBlocked(admin().indices().prepareGetSettings("test"), IndexMetadata.INDEX_METADATA_BLOCK);
    ...
    public void testCreateAndDeleteIndexConcurrently() throws InterruptedException {
        ...
        admin().indices().prepareDelete("test").execute(new ActionListener<AcknowledgedResponse>() { // this happens async!!!
            @Override
                ...
                Thread thread = new Thread() {
                    ...
                    public void run() {
                        try {
                            ...
                            assertAcked(admin().indices().prepareDelete("test").get());
                        } finally {
                            latch.countDown();
                        }
                    }
    ...
    public void testRestartIndexCreationAfterFullClusterRestart() throws Exception {
        clusterAdmin().prepareUpdateSettings()
            ...
            .get();
        admin().indices().prepareCreate("test").setWaitForActiveShards(ActiveShardCount.NONE).setSettings(indexSettings()).get();
    ...
    public void testFailureToCreateIndexCleansUpIndicesService() {
        ...
        Settings settings = Settings.builder()
            ...
            .build();
        assertAcked(
            admin().indices().prepareCreate("test-idx-1").setSettings(settings).addAlias(new Alias("alias1").writeIndex(true)).get()
        );

        assertRequestBuilderThrows(
            admin().indices().prepareCreate("test-idx-2").setSettings(settings).addAlias(new Alias("alias1").writeIndex(true)),
    ...
    public void testDefaultWaitForActiveShardsUsesIndexSetting() throws Exception {
        ...
        Settings settings = Settings.builder()
            ...
            .put(IndexMetadata.INDEX_NUMBER_OF_REPLICAS_SETTING.getKey(), numReplicas)
            .build();
        assertAcked(admin().indices().prepareCreate("test-idx-1").setSettings(settings).get());

        // all should fail
        settings = Settings.builder().put(settings).put(SETTING_WAIT_FOR_ACTIVE_SHARDS.getKey(), "all").build();
        assertFalse(admin().indices().prepareCreate("test-idx-2").setSettings(settings).setTimeout("100ms").get().isShardsAcknowledged());
        ...
        assertFalse(admin().indices().prepareCreate("test-idx-3").setSettings(settings).setTimeout("100ms").get().isShardsAcknowledged());
    }
