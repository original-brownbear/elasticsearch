...
public class ShrinkIndexIT extends ESIntegTestCase {
    ...
    public void testCreateShrinkIndexToN() {
        ...
        assertAcked(
            client().admin()
                ...
                .setSettings(shardsAndReplicas(shardSplits[1], 0).putNull("index.blocks.write").build())
                .get()
        );
        ensureGreen();
        ...
        assertAcked(
            client().admin()
                ...
                .setSettings(
                    shardsAndReplicas(shardSplits[2], 0).putNull("index.blocks.write")
    ...
    public void testShrinkIndexPrimaryTerm() throws Exception {
        ...
        final Settings shrinkSettings = shardsAndReplicas(numberOfTargetShards, 0).build();
    ...
    public void testCreateShrinkWithIndexSort() throws Exception {
        ...
        IllegalArgumentException exc = expectThrows(
            ...
            () -> client().admin()
                .indices()
                .prepareResizeIndex("source", "target")
                .setSettings(shardsAndReplicas(2, 0).put("index.sort.field", "foo").build())
                .get()
        );
        ...
        assertAcked(
            client().admin()
                .indices()
                .prepareResizeIndex("source", "target")
                .setSettings(shardsAndReplicas(2, 0).putNull("index.blocks.write").build())
                .get()
        );
        ensureGreen();
        assertNoResizeSourceIndexSettings("target");
    ...
    public void testShrinkThenSplitWithFailedNode() throws Exception {
        ...
        assertAcked(
            client().admin()
                ...
                .setSettings(
                    shardsAndReplicas(shardCount, 0).putNull(
                        IndexMetadata.INDEX_ROUTING_REQUIRE_GROUP_SETTING.getConcreteSettingForNamespace("_name").getKey()
                    ).build()
                )
