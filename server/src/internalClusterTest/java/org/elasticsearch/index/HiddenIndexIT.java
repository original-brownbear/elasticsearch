...
public class HiddenIndexIT extends ESIntegTestCase {
    ...
    public void testHiddenIndexSearch() {
        assertAcked(admin().indices().prepareCreate("hidden-index").setSettings(Settings.builder().put("index.hidden", true).build()));
    ...
    public void testGlobalTemplatesDoNotApply() {
        assertAcked(
            admin().indices().preparePutTemplate("a_global_template").setPatterns(List.of("*")).setMapping("foo", "type=text").get()
        );
        assertAcked(
            admin().indices().preparePutTemplate("not_global_template").setPatterns(List.of("a*")).setMapping("bar", "type=text").get()
        );
        assertAcked(
            admin().indices()
        ...
        assertAcked(
            admin().indices().preparePutTemplate("unused_template").setPatterns(List.of("not_used")).setMapping("foobar", "type=text").get()
        );

        assertAcked(
            admin().indices().prepareCreate("a_hidden_index").setSettings(Settings.builder().put("index.hidden", true).build()).get()
        );

        GetMappingsResponse mappingsResponse = admin().indices().prepareGetMappings("a_hidden_index").get();
    ...
    public void testNonGlobalTemplateCanMakeIndexHidden() {
        ...
        assertAcked(admin().indices().prepareCreate("my_hidden_pattern1").get());
        GetSettingsResponse getSettingsResponse = admin().indices().prepareGetSettings("my_hidden_pattern1").get();
    ...
    public void testAliasesForHiddenIndices() {
        ...
        assertAcked(admin().indices().prepareCreate(hiddenIndex).setSettings(Settings.builder().put("index.hidden", true).build()).get());
        ...
        final GetAliasesRequestBuilder req = admin().indices().prepareGetAliases(visibleAlias);
        ...
        response = admin().indices().prepareGetAliases("alias*").get();
        assertThat(response.getAliases().get(hiddenIndex), hasSize(1));
        assertThat(response.getAliases().get(hiddenIndex).get(0).alias(), equalTo(visibleAlias));
        assertThat(response.getAliases().get(hiddenIndex).get(0).isHidden(), nullValue());
        ...
        response = admin().indices().prepareGetAliases(hiddenAlias).get();
        assertThat(response.getAliases().get(hiddenIndex), hasSize(1));
        ...
        response = admin().indices().prepareGetAliases("alias*").setIndicesOptions(IndicesOptions.strictExpandOpen()).get();
