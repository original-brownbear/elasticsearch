...
public class CopyToMapperIntegrationIT extends ESIntegTestCase {
    public void testDynamicTemplateCopyTo() throws Exception {
        assertAcked(admin().indices().prepareCreate("test-idx").setMapping(createDynamicTemplateMapping()));
        ...
        admin().indices().prepareRefresh("test-idx").execute().actionGet();
    ...
    public void testDynamicObjectCopyTo() throws Exception {
        ...
        assertAcked(admin().indices().prepareCreate("test-idx").setMapping(mapping));
