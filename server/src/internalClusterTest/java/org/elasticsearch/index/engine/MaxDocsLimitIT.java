...
public class MaxDocsLimitIT extends ESIntegTestCase {
    ...
    public void testMaxDocsLimit() throws Exception {
        ...
        if (randomBoolean()) {
            admin().indices().prepareFlush("test").get();
        }
    ...
    public void testMaxDocsLimitConcurrently() throws Exception {
        internalCluster().ensureAtLeastNumDataNodes(1);
        assertAcked(admin().indices().prepareCreate("test").setSettings(Settings.builder().put(IndexMetadata.SETTING_NUMBER_OF_SHARDS, 1)));
