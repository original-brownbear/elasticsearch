...
public class InternalEngineMergeIT extends ESIntegTestCase {
    ...
    public void testMergesHappening() throws Exception {
        ...
        for (int i = 0; i < rounds; ++i) {
            ...
            IndicesStatsResponse stats = admin().indices().prepareStats("test").setSegments(true).setMerge(true).get();
            logger.info(
        ...
        assertBusy(() -> {
            IndicesStatsResponse stats = admin().indices().prepareStats().setSegments(true).setMerge(true).get();
            logger.info(
